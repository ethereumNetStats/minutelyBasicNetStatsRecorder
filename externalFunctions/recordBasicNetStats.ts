...
export const recordBasicNetStats = async (timeRangeArray: timeRangeArray, socketClient: Socket<ClientToServerEvents>, durationInSec: number) => {
    ...
    for (let timeRange of timeRangeArray) {
        ...
        socketClient.emit('minutelyBasicNetStatsRecorded', queryData);
        console.log(`${currentTimeReadable()} | Emit : 'minutelyBasicNetStatsRecorded' | To : socketServer`);

    }

    // 処理時間の計測終了
    let processEnd: number = performance.now();
    console.log(`${currentTimeReadable()} | End : Recording minutely basic net stats process. | Process time : ${((processEnd - processStart) / 1000).toString().slice(0, -12)} sec.`);
